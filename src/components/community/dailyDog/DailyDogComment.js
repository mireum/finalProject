...
function DailyDogComment(props) {
  ...
  useEffect(() => {
    const dailyDogCommentData = async () => {
      try {
        const responseComment = await axios.get(`${process.env.SERVER_DOMAIN}/community/daily/comment/${item._id}`);
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      const date = new Date();
      await axios.post(`${process.env.SERVER_DOMAIN}/community/daily/comment/insert`, { postId: item._id , comment: newComment, date, author: user.signUserNicname, authorId: user._id }); 
      const responseComment = await axios.get(`${process.env.SERVER_DOMAIN}/community/daily/comment/${item._id}`); 
      setComments(responseComment.data);
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteComment = async (id) => {
    try {
      await axios.delete(`${process.env.SERVER_DOMAIN}/community//daily/comment/delete/${id}`);
      const responseComment = await axios.get(`${process.env.SERVER_DOMAIN}/community/daily/comment/${item._id}`); 
      setComments(responseComment.data);
    } catch (err) {
      console.error(err);
